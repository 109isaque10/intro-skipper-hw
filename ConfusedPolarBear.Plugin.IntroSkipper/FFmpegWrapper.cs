...
public static class FFmpegWrapper
{
    ...
    public static TimeRange[] DetectSilence(QueuedEpisode episode, int limit)
    {
        ...
        var args = string.Format(
            ...
            "-vn -sn -dn " +
                "-i \"{0}\" -to {1} -af \"silencedetect=noise={2}dB:duration=0.1\" -f null -hwaccel cuda -hwaccel_output_format cuda -",
            episode.Path,
            limit,
        ...
        int minimum)
    {
        ...
        var args = string.Format(
            CultureInfo.InvariantCulture,
            "-ss {0} -i \"{1}\" -to {2} -an -dn -sn -vf \"blackframe=amount=50\" -f null -hwaccel cuda -hwaccel_output_format cuda -",
            range.Start,
            episode.Path,
    ...
    private static uint[] Fingerprint(QueuedEpisode episode, AnalysisMode mode, int start, int end)
    {
        ...
        var args = string.Format(
            CultureInfo.InvariantCulture,
            "-ss {0} -i \"{1}\" -to {2} -ac 2 -f chromaprint -fp_format raw -hwaccel cuda -hwaccel_output_format cuda -",
            start,
            episode.Path,
            end - start);
